      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
        {filteredSlots.map((slot) => {
          ...
          return (
            <article
              key={slot.id}
              className="group relative rounded-2xl border border-white/10 bg-black/60 overflow-hidden shadow-[0_18px_45px_rgba(0,0,0,0.75)] hover:border-neonPurple/60 hover:-translate-y-1 hover:shadow-[0_26px_70px_rgba(0,0,0,0.9)] transition"
            >
              {/* BACKGROUND IMAGE ČEZ CEL BOX – poravnana TOP */}
              {hasImage && (
                <div
                  className="absolute inset-0 bg-cover bg-top opacity-55 group-hover:opacity-75 transition-opacity duration-300"
                  style={{
              ...
              {/* DARK GRADIENT, DA JE TEKST BERLJIV */}
              <div className="absolute inset-0 bg-gradient-to-b from-black/85 via-black/55 to-black/90" />

              {/* CONTENT */}
              <div className="relative p-4 flex flex-col justify-between h-full">
                {/* TOP: naslov + provider + tagi */}
                <div className="space-y-2">
                  <div>
                    <h2 className="text-base md:text-lg font-semibold text-white line-clamp-2 drop-shadow-md">
                      {slot.name}
                    </h2>
                    <p className="mt-1 text-xs text-gray-200 drop-shadow">
                      {slot.provider}
                    </p>
                  </div>
